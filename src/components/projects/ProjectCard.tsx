...
export function ProjectCard({ project }: { project: Project }) {
  return (
    <motion.article
      ...
      className="group shadow-soft hover:shadow-lift h-90 relative overflow-hidden rounded-2xl border border-border bg-card p-2 transition-shadow duration-300"
    >
      <div
        className={`relative aspect-[16/10] overflow-hidden rounded-xl bg-gradient-to-br ${project.gradient}`}
      >
        <img src={project.image} alt={project.name} className="object-cover transition-transform duration-500 group-hover:scale-105" />
        ...
        <motion.div
          aria-hidden
          className=" absolute -inset-x-8 -top-16 h-40 rounded-full opacity-30 blur-3xl transition-opacity duration-500 group-hover:opacity-60"
        />
      ...
      <div className="p-3.5">
        ...
        <ul className="mt-3.5 flex flex-wrap gap-1.5">
          {project.tags.map((tag) => (
            <li
              key={tag}
              className="rounded-lg border border-border bg-surface px-2 py-0.5 text-[0.6rem] text-muted-foreground"
            >
              {tag}
            </li>
          ))}
